...
def results_generator(exch_list,pair_list,usd_amounts):
    ...
    res_df = pd.DataFrame(columns=["exchange","pair","selling_start_date","selling_end_date","time_until_filled"])
    col_list = []
    for usd_amount in usd_amounts:
        sc_name = "sell_cost_"+str(usd_amount)
        sl_name = "slippage_"+str(usd_amount)
        ...
        for pair in pair_list:
            ...
            try:
                ...
                for usd_amount in usd_amounts:
                    ...
                    res_dict["sell_cost_"+str(usd_amount)] = sell_cost * volume #sell cost = average slippage per unit * volume
                    res_dict["slippage_"+str(usd_amount)] = slip
            except Exception as e: #if exception -> set field as NaN
                for usd_amount in usd_amounts:
                    res_dict["sell_cost_"+str(usd_amount)] = np.nan
                    res_dict["slippage_"+str(usd_amount)] = np.nan
